/*------------------------------------------------------------------------------------------------------/
| Program : ACA_OWNERSUBMITTAL_BEFORE.js 
| Event   : ACA Check to make sure only one submittal per public user account
|
    ...
    if (typeof(RELATEDAPPLICATIONS) == "object") {
        for (var j in RELATEDAPPLICATIONS) {
            ...
            if (!licId.getOutput()) {
                ...
                showMessage = true;
                //showDebug = false;
                comment(thisRow["Application ID"] + " is not valid.  Please enter a valid application ID");
            }
        }
    }
} catch (err) {
