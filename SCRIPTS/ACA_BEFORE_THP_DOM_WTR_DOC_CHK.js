/*------------------------------------------------------------------------------------------------------/
| Program : ACA_BEFORE_THP_DOM_WTR_DOC_CHK.js
...
try {
	...
	var notifLetter = {condition : "Notification Letter", document : "Notification Letter"};
	var newsPaperNotif = {condition : "News Paper Posting", document : "News Paper Posting"};
	//r = getReqdDocs("Owner");
	var r = [];
	if(AInfo["DOM_LANDOWNER"]=="Yes"){
		if(AInfo["DOM_EXAMPTION"]!="Yes"){
			r.push(notifLetter);
			r.push(newsPaperNotif);
		}else{
			if(AInfo["DOM_NEWSPAPER"]=="CHECKED" && AInfo["DOM_LETTER"]!="CHECKED"){
				r.push(notifLetter);
			}
			if(AInfo["DOM_LETTER"]=="CHECKED" && AInfo["DOM_NEWSPAPER"]!="CHECKED"){
				r.push(newsPaperNotif);
			}
		}
	}
	...
	showDebug =true;
	logDebug("An error has occurred in ACA_BEFORE_THP_DOM_WTR_DOC_CHK: Main function: " + err.message);
	logDebug(err.stack);
